...
class AssertModelNumQueriesContext(CaptureQueriesContext):
    ...
    def __init__(self, expected_model_counts=None, strict=True, ignore=None, test_case=None, connection=None, verbosity=0):
        self.strict = strict
        self.ignore_models = {normalize_key(instance) for instance in ignore} if ignore else set()
        self.verbosity = verbosity
        self.test_case = test_case
        self.expected_model_counts = parse_counts(expected_model_counts) if expected_model_counts is not None else None
    ...
    def __enter__(self):
        ...
        if self.expected_model_counts is None:
            raise ExpectedModelCountsNotSet("The expected model counts can be passed as a constructor argument or as a context manager argument.")
...
class ModelNumQueriesHelper:
    def assertModelNumQueries(self, expected_model_counts, func=None, *args, using=DEFAULT_DB_ALIAS, **kwargs):
        conn = connections[using]

        context = AssertModelNumQueriesContext(expected_model_counts=expected_model_counts, test_case=self, connection=conn)
        if func is None:
            return context
